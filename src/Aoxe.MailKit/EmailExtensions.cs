...
{
    public static MimeMessage ToMimeMessage(this Email.Abstractions.Models.Email email)
    {
        ...
        body.AddRange(
            email.Attachments.Select(attachment =>
            {
        ...
        mimeMessage.To.AddRange(
            email.Recipients.To.Select(to => new MailboxAddress(to.Name, to.Address))
        );
        mimeMessage.Cc.AddRange(
            email.Recipients.Cc.Select(cc => new MailboxAddress(cc.Name, cc.Address))
        );
